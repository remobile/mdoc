                            ...
                            menu.pages.map((o, k) => (
                                <tr key={k}>
                                    <td style={{backgroundColor: config.colors.primaryColor, borderTop: 'solid 1px gray'}}>
                                        <a href={getLink(config.baseUrl, o.path, o.id)} key={k} target='_self' style={ current.id===o.id ? { color: config.colors.activeColor }: {} }>{o.name}</a>
                                    </td>
                                </tr>
                            ))
                        }
...
class HeaderNav extends React.Component {
    makeLinks(menu, key) {
        ...
        return (
            <li key={key}>
                {
                    mainPage &&
                    <a href={getLink(config.baseUrl, mainPage, mainPageId)} target={menu.external ? '_blank' : '_self'} style={ menuId===id ? {color: config.colors.activeColor} : {} }>
                        {name}
                    </a>
                    ||
    ...
    render() {
        ...
        const { baseUrl, headerIcon, title, menus } = config;
        return (
                ...
                <div className="headerWrapper wrapper">
                    <header>
                        <a href={baseUrl}>
                            {
                                headerIcon &&
                                <img className="logo" src={baseUrl + headerIcon} />
                            }
