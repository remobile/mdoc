...
class SingleDocLayout extends React.Component {
    render() {
        ...
        const { projectName, highlight ={} }  = config;
        ...
        return (
            <html>
                <head>
                    ...
                    { hasCode && <link rel="stylesheet" href={`https://cdnjs.cloudflare.com/ajax/libs/highlight.js/${hl.version}/styles/${hl.theme}.min.css`} /> }
                    { hasJquery && <script type="text/javascript" src={`/${projectName}/js/jquery-1.4.4.min.js`} /> }
                    { hasUnderscore && <script type="text/javascript" src={`/${projectName}/js/underscore-min.js`} /> }
